...
import { Plus, MessageSquare, Trash2, Settings, Github, User as UserIcon, LogOut, X } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({
    ...
    return (
        <>
            ...
            <aside className={`fixed md:relative z-40 h-full transition-all duration-300 ease-in-out bg-slate-100 dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col overflow-hidden shrink-0 
                ${isOpen ? 'w-72 translate-x-0' : 'w-0 -translate-x-full md:translate-x-0'}`}>
                <div className="p-4 flex items-center gap-2">
                    <button onClick={handleNew}
                        className="flex-1 flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg py-3 px-4 transition-colors font-medium shadow-lg shadow-indigo-500/20">
                        <Plus size={18} />
                        New Chat
                    </button>
                    {isOpen && (
                        <button 
                            onClick={onClose}
                            className="md:hidden p-3 text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 transition-colors"
                        >
                            <X size={20} />
                        </button>
                    )}
                ...
                <div className="flex-1 overflow-y-auto px-3 space-y-1">
                    {conversations.length === 0 ? (
                        ...
                        conversations.sort((a, b) => b.updatedAt - a.updatedAt).map((conv) => (
                            <div key={conv.id} onClick={() => handleSelect(conv.id)} className={`group flex items-center gap-3 px-3 py-3 rounded-lg cursor-pointer transition-all ${activeId === conv.id
